...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://kena-kati-server.vercel.app/users');
    ...
    const handleDeleteUser = user => {
        fetch(`https://kena-kati-server.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleMakeAdmin = id => {
        fetch(`https://kena-kati-server.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
